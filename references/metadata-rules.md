...
文档需要向读者或机器陈述两个及以上（≥2）可独立列举的属性（版本、状态、日期、适用范围、来源、关联文档、负责人等）时，必用元数据标记；只有单一属性时写进正文即可。
...
- frontmatter 中的路径值一律相对仓库根目录书写。
- 数组值用 YAML 列表；布尔用小写 `true` / `false`（渲染为复选框），数值不加引号；`yes/no/on/off` 只按普通文本处理，不要用。
...
| `related` | 关联文档 | 相对仓库根目录的路径列表（如 `references/render-contract.md`） |
